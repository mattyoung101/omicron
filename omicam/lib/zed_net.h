...
#define INVALID_SOCKET -1 // I have no idea if this will work or not
...
ZED_NET_DEF int zed_net_check_would_block(zed_net_socket_t *socket) {
    ...
    if (socket->non_blocking && !socket->ready) {
        // FIXME this will break things, temporary workaround since library authors seem to have neglected Linux
//		writefd.fd_count = 1;
